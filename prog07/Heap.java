...
public class Heap <E> extends AbstractQueue<E> {
  ...
  /** @return the index of the element i*/
  ...
  /** @return true if i == root index (0) */
  private boolean isRoot (int i) {
    return i == 0;
  }

  /** @return true index i is >= size of tree (array / list) */
  ...
  {//{{{
    ...
    /* Remove the last item from the ArrayList and place it into
       the first position. */
    lastElement = theData.remove(theData.size()-1);
    
    theData.set(0,lastElement);
    		...
    		!isNull(getRight(index)) && compare(index, getRight(index)) >0 )
    {
      ...
      // If the right child is there and is smaller than the left child...
    	if(!isNull(getLeft(index)) && !isNull(getRight(index)) 
    			&& compare(getRight(index),getLeft(index)) < 0)
    	{
  ...
  {//{{{
    ...
    // item at index.
    	E lastElement = theData.remove(theData.size()-1);
    	theData.set(index,lastElement);
    ...
    // as in offer.  Otherwise, swap it downward as in poll().
    	...
    	else
    	{
            ...
            {
            	if (!isNull(getLeft(index)) &&  !isNull(getRight(index)) && compare(getLeft(index), getRight(index)) > 0)      
            	{
            		swap (index, getRight(index));
            		index = getRight(index);
            	}
